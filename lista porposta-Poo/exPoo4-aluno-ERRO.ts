...
// resolução usando um vetor (preciso entender se eh possivel limitar esse vetor)
class Alunos {
    nome: string;
    matricula: number;
    notas:number[];

    constructor(nome:string, matricula:number, notas:number[]){
        this.nome = nome;
    ...
    media():number{
        ...
        // isso pode mas por algum motivo nao usa os parenteses se nao da erro
        return mediaAlunos/this.notas.length;
    }

    situação(): string{
        // procurar saber pq isso ta errado
        if (this.media() >= 7){
